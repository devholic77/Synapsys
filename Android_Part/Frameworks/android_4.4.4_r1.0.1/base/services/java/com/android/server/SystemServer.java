...
class ServerThread {
	...
	public void initAndLoop() {
		...
		// Check whether we failed to shut down last time we tried.
		{
			...
			if (shutdownAction != null && shutdownAction.length() > 0) {
				...
				if (shutdownAction.length() > 1) {
					reason = shutdownAction.substring(1,
							shutdownAction.length());
				} else {
					reason = null;
				}
		...
		final boolean headless = "1".equals(SystemProperties.get(
		...
		wmHandler.post(new Runnable() {
			...
			public void run() {
				...
				// android.util.Log.DEBUG, TAG,
				// android.util.Log.LOG_ID_SYSTEM));
				android.os.Process
						.setThreadPriority(android.os.Process.THREAD_PRIORITY_DISPLAY);
				android.os.Process.setCanSelfBackground(false);

				// For debug builds, log event loop stalls to dropbox for
				// analysis.
		...
		try {
			// Wait for installd to finished starting up so that it has a chance
			// to
			// create critical directories such as /data/user with the
			// appropriate
			// permissions. We need this to complete before we initialize other
			// services.
			...
			Slog.e("System", "******************************************");
			Slog.e("System", "************ Failure starting bootstrap service",
					e);
		}

		boolean disableStorage = SystemProperties.getBoolean(
		...
		try {
			...
			power.init(context, lights, ActivityManagerService.self(), battery,
					BatteryStatsService.getService(), ActivityManagerService
							.self().getAppOpsService(), display);
		...
		if (factoryTest != SystemServer.FACTORY_TEST_LOW_LEVEL) {
			// if (!disableNonCoreServices) { // TODO: View depends on these;
			// mock them?
			if (true) {
				try {
					...
					ServiceManager
							.addService(Context.INPUT_METHOD_SERVICE, imm);
				} catch (Throwable e) {
		...
		if (factoryTest != SystemServer.FACTORY_TEST_LOW_LEVEL) {
			if (!disableStorage
					&& !"0".equals(SystemProperties
							.get("system_init.startmountservice"))) {
				try {
					/*
			...
			if (!disableNonCoreServices) {
				...
				try {
					...
					ServiceManager.addService(Context.DEVICE_POLICY_SERVICE,
							devicePolicy);
				} catch (Throwable e) {
			...
			if (!disableNetwork) {
				try {
					...
					networkManagement = NetworkManagementService
							.create(context);
					ServiceManager.addService(
			...
			if (!disableNonCoreServices) {
				try {
					...
					ServiceManager.addService(
							Context.TEXT_SERVICES_MANAGER_SERVICE, tsms);
				} catch (Throwable e) {
			...
			if (!disableNetwork) {
				try {
					...
					networkStats = new NetworkStatsService(context,
							networkManagement, alarm);
					ServiceManager.addService(Context.NETWORK_STATS_SERVICE,
							networkStats);
				} catch (Throwable e) {
				...
				try {
					...
					networkPolicy = new NetworkPolicyManagerService(context,
							ActivityManagerService.self(), power, networkStats,
							networkManagement);
					ServiceManager.addService(Context.NETWORK_POLICY_SERVICE,
							networkPolicy);
				} catch (Throwable e) {
				...
				try {
					...
					ServiceManager
							.addService(Context.WIFI_P2P_SERVICE, wifiP2p);
				} catch (Throwable e) {
				...
				try {
					...
					ServiceManager.addService(Context.NSD_SERVICE,
							serviceDiscovery);
				} catch (Throwable e) {
			...
			try {
				...
				ServiceManager.addService(Context.DROPBOX_SERVICE,
						new DropBoxManagerService(context, new File(
								"/data/system/dropbox")));
			} catch (Throwable e) {
			...
			if (!disableNonCoreServices
				...
				try {
					...
					if (!headless) {
						...
						ServiceManager.addService(Context.WALLPAPER_SERVICE,
								wallpaper);
					}
				} catch (Throwable e) {
			...
			if (!disableMedia
					&& !"0".equals(SystemProperties
							.get("system_init.startaudioservice"))) {
				try {
					Slog.i(TAG, "Audio Service");
					ServiceManager.addService(Context.AUDIO_SERVICE,
			...
			if (!disableNonCoreServices) {
				...
				try {
					...
					ServiceManager.addService(Context.APPWIDGET_SERVICE,
							appWidget);
				} catch (Throwable e) {
			...
			try {
				...
				// is false, because it is this service that detects system
				// property change and
			...
			if (!disableNonCoreServices) {
				try {
					...
					ServiceManager.addService(Context.MEDIA_ROUTER_SERVICE,
							mediaRouter);
				} catch (Throwable e) {
			...
			if (!disableSynapsys) {
				try {
					...
					ServiceManager.addService(Context.SYNAPSYS_SERVICE,
							synapsysManager);
				} catch (Throwable e) {
			...
			// Update the configuration for this context by hand, because we're
			// going
			// to start using it before the config change done in
			// wm.systemReady() will
			// propagate to it.
			...
			// started launching the initial applications), for us to complete
			// our
			// initialization.
			ActivityManagerService.self().systemReady(new Runnable() {
				public void run() {
					...
					// It is now okay to let the various system services start
					// their
					// third party code...
					...
					try {
						...
						reportWtf(
								"Notifying CommonTimeManagementService running",
								e);
					}
					try {
						...
						reportWtf(
								"Notifying TextServicesManagerService running",
								e);
					}
					try {
						if (dreamyF != null)
...
public class SystemServer {
	...
	public static void main(String[] args) {
		...
		/*
		 * In case the runtime switched since last boot (such as when the old
		 * runtime was removed in an OTA), set the system property so that it is
		 * in sync. We can't do this in libnativehelper's JniInvocation::Init
		 * code where we already had to fallback to a different runtime because
		 * it is running as root and we need to be the system user to set the
		 * property. http://b/11463182
		 */
		SystemProperties.set("persist.sys.dalvik.vm.lib", VMRuntime
				.getRuntime().vmLibrary());
